import { ls as TimestampQuery, uc as warnOnce } from "./three.core-B2AdjTlh.js";
import { t as InspectorBase } from "./three.webgpu-fBhnPIcS.js";
		...
		this._resolveTimestampPromise = new Promise((resolve) => {
			requestAnimationFrame(async () => {
				...
				for (const frameId of frameIds) {
					...
					if (frame !== null) {
						if (frame.resolvedCompute === false) if (frame.computes.length > 0) {
							if (computeFrames.includes(frameId)) {
								for (const stats of frame.computes) if (renderer.backend.hasTimestamp(stats.uid)) stats.gpu = renderer.backend.getTimestamp(stats.uid);
								else {
									stats.gpu = 0;
						...
						if (frame.resolvedRender === false) if (frame.renders.length > 0) {
							if (renderFrames.includes(frameId)) {
								for (const stats of frame.renders) if (renderer.backend.hasTimestamp(stats.uid)) stats.gpu = renderer.backend.getTimestamp(stats.uid);
								else {
									stats.gpu = 0;
									stats.gpuNotAvailable = true;
								}
